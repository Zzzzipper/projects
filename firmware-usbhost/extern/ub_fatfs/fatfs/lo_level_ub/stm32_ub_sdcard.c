...
BYTE local_buff[_MAX_SS] __attribute__ ((section (".ccmram")));
...
int MMC_disk_read(BYTE *buff, DWORD sector, BYTE count)
{
  ...
  SD_ReadMultiBlocks(buff, sector << 9, _MAX_SS, 1);
...
//--------------------------------------------------------------
int MMC_disk_write_async(const BYTE *buff, DWORD sector, BYTE count)
{
  int ret_wert=-1;
  ...
  memcpy(local_buff, buff, _MAX_SS);

  SD_WriteMultiBlocks((BYTE *) local_buff, sector << 9, _MAX_SS, 1);
...
int MMC_disk_write(const BYTE *buff, DWORD sector, BYTE count)
{
  ...
  SD_WriteMultiBlocks((BYTE *)buff, sector << 9, _MAX_SS, 1);
