...
USBH_StatusTypeDef USBH_SelectInterface(USBH_HandleTypeDef *phost, uint8_t interface)
{
  ...
  {
    USBH_ErrLog ("Cannot Select This Interface (%i>=%i)", interface, phost->device.CfgDesc.bNumInterfaces);
    status = USBH_FAIL;
  }
  return status;
}
...
USBH_StatusTypeDef USBH_Process(USBH_HandleTypeDef *phost) {
	...
	case HOST_DEV_ATTACHED:
		...
		USBH_UsrLog(">    --dev.address %d", phost->device.address);
		USBH_UsrLog(">    --dev.speed %d", phost->device.speed);
		USBH_UsrLog(">    --pipe size %d", phost->Control.pipe_size);
	...
	case HOST_CHECK_CLASS:
		...
		if (phost->ClassNumber == 0) {
			...
			USBH_UsrLog(">    --USB Check Class Code=0x%X, Pid=0x%.4X, Vid=0x%.4X",
			...
			for (idx = 0; idx < USBH_MAX_NUM_SUPPORTED_CLASS; idx++) {
				USBH_UsrLog(">    --Comparison Code=0x%X, Pid=0x%.4X, Vid=0x%.4X"
											, phost->pClass[idx]->ClassCode
											, phost->pClass[idx]->Pid
											, phost->pClass[idx]->Vid);
				if (phost->pClass[idx]->ClassCode
					...
					break;
#else
					if ((phost->pClass[idx]->ClassCode != USB_CLASS_CODE_VENDOR_SPECIFIC) ||
							(phost->pClass[idx]->Pid == phost->device.DevDesc.idProduct && phost->pClass[idx]->Vid == phost->device.DevDesc.idVendor))
					{
						phost->pActiveClass = phost->pClass[idx];
					}
#endif
			...
			if (phost->pActiveClass != NULL) {
				...
				char msg[80];
				sprintf(msg,
						"No registered class for this device. Interface class: %X",
						phost->device.CfgDesc.Itf_Desc[0].bInterfaceClass);
	...
	case HOST_CLASS_REQUEST:
		USBH_UsrLog(">    --HOST_CLASS_REQUEST");
		/* process class standard control requests state machine */
		if (phost->pActiveClass != NULL) {
			status = phost->pActiveClass->Requests(phost);
			USBH_UsrLog(">       --Status active class request: %d", status);
	...
	case HOST_CLASS:
		...
		if (phost->pActiveClass != NULL) {
			USBH_StatusTypeDef result = phost->pActiveClass->BgndProcess(phost);
			if (result == USBH_INTERNAL_RESET) {
				USBH_UsrLog(">    --Request ReEnumerate from driver");
	...
	case HOST_DEV_DISCONNECTED:
		...
		USBH_UsrLog(">    --HOST_DEV_DISCONNECTED..");
...
static USBH_StatusTypeDef USBH_HandleEnum (USBH_HandleTypeDef *phost)
{
  ...
  case ENUM_GET_FULL_DEV_DESC:
    ...
    {
      USBH_UsrLog(">    PID: %x..", phost->device.DevDesc.idProduct );
      USBH_UsrLog(">    VID: %x..", phost->device.DevDesc.idVendor );
