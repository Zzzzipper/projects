...
extern uint32_t USB_GetCurrentAndLastTimeDiff(uint32_t lastTimeMs);
extern uint32_t USB_GetCurrentMs();

static uint8_t usb_cdc_rx_buff[USB_CDC_DEFAULT_RX_TX_BUFFER_SIZE];
static uint8_t usb_cdc_tx_buff[USB_CDC_DEFAULT_RX_TX_BUFFER_SIZE];
...
{	
  ...
  else
  {
    ...
    {    
      ...
      CDC_Handle->rxLock = true;
      CDC_Handle->txLock = true;
...
static USBH_StatusTypeDef USBH_CDC_Process (USBH_HandleTypeDef *phost)
{
	...
	return USBH_CDC_Process2(phost);
}

USBH_StatusTypeDef USBH_CDC_Process2 (USBH_HandleTypeDef *phost)
{
  ...
  case CDC_SET_LINE_CODING_STATE:
    req_status = SetLineCoding(phost, handle->pUserLineCoding);
    
    if(req_status == USBH_OK)
    {
      handle->state = CDC_GET_LAST_LINE_CODING_STATE; 
    }
    
    else if(req_status != USBH_BUSY)
    {
      handle->state = CDC_ERROR_STATE; 
    }
    break;
    
    
  case CDC_GET_LAST_LINE_CODING_STATE:
    req_status = GetLineCoding(phost, &(handle->LineCoding));
    
    if(req_status == USBH_OK)
    {
      handle->state = CDC_IDLE_STATE; 
      
      if((handle->LineCoding.b.bCharFormat == handle->pUserLineCoding->b.bCharFormat) && 
         (handle->LineCoding.b.bDataBits == handle->pUserLineCoding->b.bDataBits) &&
         (handle->LineCoding.b.bParityType == handle->pUserLineCoding->b.bParityType) &&
         (handle->LineCoding.b.dwDTERate == handle->pUserLineCoding->b.dwDTERate))
      {
...
USBH_StatusTypeDef  USBH_CDC_Transmit(USBH_HandleTypeDef *phost, uint8_t *pbuff, uint32_t length)
{
  ...
  if((handle->state == CDC_IDLE_STATE) || (handle->state == CDC_TRANSFER_DATA))
  {
    ...
    handle->state = CDC_TRANSFER_DATA;
    handle->data_tx_state = CDC_SEND_DATA;
    Status = USBH_OK;
#if (USBH_USE_OS == 1)
      osMessagePut ( phost->os_event, USBH_CLASS_EVENT, 0);
#endif      
...
USBH_StatusTypeDef  USBH_CDC_Receive(USBH_HandleTypeDef *phost, uint8_t *pbuff, uint32_t length)
{
  ...
  if((handle->state == CDC_IDLE_STATE) || (handle->state == CDC_TRANSFER_DATA))
  {
    ...
    handle->state = CDC_TRANSFER_DATA;
    handle->data_rx_state = CDC_RECEIVE_DATA;     
    Status = USBH_OK;
#if (USBH_USE_OS == 1)
      osMessagePut ( phost->os_event, USBH_CLASS_EVENT, 0);
#endif        
  ...
  case CDC_SEND_DATA:
    if(handle->TxDataLength > handle->DataItf.OutEpSize)
    {
      USBH_BulkSendData (phost,
                         handle->pTxData, 
                         handle->DataItf.OutEpSize, 
                         handle->DataItf.OutPipe,
                         1);
    }
    else
    {
    ...
    handle->data_tx_state = CDC_SEND_DATA_WAIT;
    
    break;
    
  case CDC_SEND_DATA_WAIT:
    
    URB_Status = USBH_LL_GetURBState(phost, handle->DataItf.OutPipe); 
...
static void CDC_ProcessReception(USBH_HandleTypeDef *phost)
{
  ...
  case CDC_RECEIVE_DATA:
    ...
    USBH_BulkReceiveData (phost,
                          handle->pRxData, 
                          handle->DataItf.InEpSize, 
                          handle->DataItf.InPipe);
    
    handle->data_rx_state = CDC_RECEIVE_DATA_WAIT;
    
    break;
    
  case CDC_RECEIVE_DATA_WAIT:
    
    URB_Status = USBH_LL_GetURBState(phost, handle->DataItf.InPipe); 
