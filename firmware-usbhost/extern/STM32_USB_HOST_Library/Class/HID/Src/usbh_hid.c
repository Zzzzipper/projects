...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost)
{
  ...
  case HID_IDLE:
  {
	USBH_StatusTypeDef result = USBH_HID_GetReport (phost, 0x01, 0, HID_Handle->pData, HID_Handle->length);

	if(result == USBH_OK)
    {
		USBH_UsrLog ("USBH_HID_GetReport OK");

		fifo_write(&HID_Handle->fifo, HID_Handle->pData, HID_Handle->length);
    }
	else
	{
		USBH_UsrLog ("USBH_HID_GetReport Error");
	}
...
uint16_t  fifo_read(FIFO_TypeDef * f, void * buf, uint16_t  nbytes)
{
	if (!f->size)
	{
		 USBH_UsrLog ("Ошибка чтения из FIFO, без инициализации!");
		 return 0;
	}
...
uint16_t  fifo_write(FIFO_TypeDef * f, const void * buf, uint16_t  nbytes)
{
	if (!f->size)
	{
		 USBH_UsrLog ("Ошибка записи в FIFO, без инициализации!");
		 return 0;
	}
