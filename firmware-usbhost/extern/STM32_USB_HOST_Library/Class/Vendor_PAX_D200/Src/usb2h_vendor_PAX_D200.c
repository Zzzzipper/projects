...
#ifdef DEVICE_USB2
/* Includes ------------------------------------------------------------------*/
#include "usb2h_vendor_PAX_D200.h"
	...
	case PAX_D200_RECEIVE_REQUEST_WAIT:
		if (phost->device.DevDesc.bcdDevice >= 0x300)
		{
			...
			if (bioPack->len + 4 > handle->realReceivedLen)
			{
				...
				{
					USBH_ErrLog("Too big request data block, resetting!");
					PAX_D200_Reset(phost);
					break;
				}
				...
				USBH_DbgLog("Request next data block, requested len: %d, real: %d", bioPack->len + 4, handle->realReceivedLen);
				break;
			}
