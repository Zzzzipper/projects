#include "Config3AuditIniter.h"
#include "utils/include/Number.h"
#include "logger/include/Logger.h"

#include <string.h>

Config3AuditIniter::Config3AuditIniter(Config3Modem *config) :
	Evadts::Parser(),
	config(config) {
}

void Config3AuditIniter::procStart() {
	LOG_DEBUG(LOG_EVADTS, "procStart");
	config->getAutomat()->shutdown();
}

void Config3AuditIniter::procComplete() {
	LOG_DEBUG(LOG_EVADTS, "complete");
	config->init();
}

bool Config3AuditIniter::procLine(char **tokens, uint16_t tokenNum) {
...
bool Config3AuditIniter::parseID4(char **tokens, uint16_t tokenNum) {
	enum {
		ID4_DecimalPoint = 1
	};
 ...
 */
bool Config3AuditIniter::parsePA1(char **tokens, uint16_t tokenNum) {
...
bool Config3AuditIniter::parsePA7(char **tokens, uint16_t tokenNum) {
