...
class Config4DeviceTest : public TestSet {
public:
	Config4DeviceTest();
	bool testSaveLoad();
};

TEST_SET_REGISTER(Config4DeviceTest);

Config4DeviceTest::Config4DeviceTest() {
	TEST_CASE_REGISTER(Config4DeviceTest, testSaveLoad);
}

bool Config4DeviceTest::testSaveLoad() {
	RamMemory memory(32000);
	TestRealTime realtime;
	Config4DeviceList config1(2, &realtime, 25000);
	config1.init(&memory);
	...
	memory.setAddress(0);
	Config4DeviceList config2(2, &realtime, 25000);
	config2.load(&memory);
