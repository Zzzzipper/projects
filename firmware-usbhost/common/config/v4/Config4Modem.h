#ifndef COMMON_CONFIG_V4_MODEM_H_
#define COMMON_CONFIG_V4_MODEM_H_

#include "config/include/StatStorage.h"
#include "config/v1/boot/Config1Boot.h"
#include "config/v2/fiscal/Config2Fiscal.h"
#include "config/v4/event/Config4EventList.h"
#include "config/v4/automat/Config4Automat.h"

class Config4Modem {
public:
	Config4Modem(Memory *memory, RealTimeInterface *realtime, StatStorage *stat);
	~Config4Modem();
	MemoryResult init();
	MemoryResult load();
	MemoryResult save();
	...
	Config4EventList *getEvents() { return &events; }
	Config4Automat *getAutomat() { return &automat; }
