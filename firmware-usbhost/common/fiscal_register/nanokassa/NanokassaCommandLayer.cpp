...
void CommandLayer::makeCheckBody() {
	...
	for(uint16_t i = 0; i < saleData->getProductNum(); i++) {
		Fiscal::Product *product = saleData->getProduct(i);
		if(i > 0) { buf << ","; }
		buf << "{\"name\": \""; convertWin1251ToJsonUnicode(product->name.get(), &buf); buf << "\"";
		buf << ",\"price\": " << context->money2value(product->price);
		buf << ",\"amount\": " << product->quantity * 1000;
		buf << ",\"vat\": " << convertTaxRate2Nanokassa(product->taxRate);
		buf << "}";
	}
	buf << "]";
#endif
