...
#include <stdint.h>
void assert_failed_handler(uint8_t* file, uint32_t line);
...
{
  /*----------Core Exceptions------------------------------------------------ */
/* FIXME: Вообще не влияет на работу то, что здесь написано.
 * Возможно, все настройки уже приходят из загрузчика.
...
{
	assert_failed_handler(file, line);
}
