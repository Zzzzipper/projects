...
void Manager::sale(Fiscal::Sale *newData, uint32_t decimalPoint) {
	LOG_DEBUG(LOG_SM, "sale " << state);
	config->getRealTime()->getDateTime(&newData->datetime);
	if(newData->price > 0 && (newData->paymentType == Fiscal::Payment_Cash || newData->paymentType == Fiscal::Payment_Cashless)) {
...
void Manager::procFreeVend(Fiscal::Sale *newData) {
	...
	newData->taxRate = Fiscal::TaxRate_NDSNone;
...
void Manager::stateRegistrationEvent(EventEnvelope *envelope) {
	...
	switch(envelope->getType()) {
		case Fiscal::Register::Event_CommandOK: stateRegistrationEventOK(); return;
		case Fiscal::Register::Event_CommandError: stateRegistrationEventError(envelope); return;
		default: LOG_ERROR(LOG_SM, "Unwaited event " << state << "," << envelope->getType());
	}
}

void Manager::stateRegistrationEventOK() {
	LOG_DEBUG(LOG_SM, "stateRegistrationEventOK");
...
void Manager::showQrCode() {
	...
	qrCodeFooter.clear();
	qrCodeFooter << saleData.name.get();
	...
	uint32_t price1 = converter.convertMasterToDevice(saleData.price);
