...
void CommandLayer::makeCheckBody() {
...
#else
	buf << "{\"id\": \"" << id << "\"";
	if(fiscal->getKkt() == ConfigFiscal::Kkt_OrangeDataEphor) { buf << ",\"key\": \"4010004\""; }
	if(fiscal->getKkt() == ConfigFiscal::Kkt_OrangeData) { buf << ",\"key\": \"" << fiscal->getINN() << "\""; }
	buf << ",\"inn\": \"" << fiscal->getINN() << "\"";
	if(strlen(fiscal->getGroup()) == 0) {
		buf << ",\"group\": \"Vend\"";
	...
	for(uint16_t i = 0; i < saleData->getProductNum(); i++) {
		Fiscal::Product *product = saleData->products.get(i);
		if(i > 0) { buf << ","; }
		buf << "{\"quantity\": " << product->quantity << ".000";
		buf << ",\"price\": "; makeMoney(product->price);
		buf << ",\"tax\": " << FiscalStorage::convertTaxRate2FN105(product->taxRate);
		buf << ",\"text\": \""; convertWin1251ToJsonUnicode(product->name.get(), &buf); buf << "\"";
	}
	buf << "}]";

	buf << ",\"checkClose\": {";
	buf << "\"payments\": [";
	buf << "{\"type\": " << getPaymentType(saleData->paymentType) << ",\"amount\": "; makeMoney(saleData->getPrice()); buf << "}";
	buf << "]";
	buf << ",\"taxationSystem\": " << convertTaxSystem(saleData->taxSystem) << "}";
	buf << ",\"automatNumber\": \""; convertWin1251ToJsonUnicode(fiscal->getAutomatNumber(), &buf); buf << "\"";
	buf << ",\"settlementPlace\": \"";  convertWin1251ToJsonUnicode(fiscal->getPointName(), &buf); buf << "\"";
