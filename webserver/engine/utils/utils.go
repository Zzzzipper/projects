...
func IsValidAlias(alias string) bool {
	...
	regexpeChars := regexp.MustCompile(`^\/([a-zA-Z0-9\/\-_\.\p{L}\d_\%\ ]+)\/?$`)
	return regexpeChars.MatchString(alias) || alias == "/"
}
...
func GetTmplSystemData(cpmod, cpsubmod string) consts.TmplSystem {
	return consts.TmplSystem{
		...
		PathCssBootstrapMin:           GetAssetsUrl(consts.AssetsBootstrapMinCss),
		PathCssMaterilaDashboard:      GetAssetsUrl(consts.AssetsMaterialDashboardCss) + "?v=2.1.2",
		...
		PathCssGoogleFonts:            "https://fonts.googleapis.com/css?family=Roboto:300,400,500,700|Roboto+Slab:400,700|Material+Icons",
		PathCssAvesomeFonts:           "https://maxcdn.bootstrapcdn.com/font-awesome/latest/css/font-awesome.min.css",
		PathMaterialDemoCss:           GetAssetsUrl(consts.AssetsMaterilaDemoCss),
		PathBootstrapMaterialDesign:   GetAssetsUrl(consts.AssetsBootstrapMaterialDesign),
		PathMaterialDashboardMinJs:    GetAssetsUrl(consts.AssetsMaterilaDashboardMinJs),
		PathPerfectScroolbarJs:        GetAssetsUrl(consts.AssetsPerfectScroolbarJs),
		PathMomentJs:                  GetAssetsUrl(consts.AssetsMomentJs),
		PathSweetalert2Js:             GetAssetsUrl(consts.AssetsSweetalert2Js),
		PathValidateJs:                GetAssetsUrl(consts.AssetsValidateJs),
		PathBootstrapWizardJs:         GetAssetsUrl(consts.AssetsBootstrapWizardJs),
		PathBootstrapSelectPickerJs:   GetAssetsUrl(consts.AssetsBootstrapSelectPickerJs),
		PathBootstrapDatetimePickerJs: GetAssetsUrl(consts.AssetsBootstrapDatetimePickerJs),
		PathJqueryDataTablesJs:        GetAssetsUrl(consts.AssetsJqueryDataTablesJs),
		PathBootstrapTagsInputJs:      GetAssetsUrl(consts.AssetsBootstrapTagsInputJs),
		PathJasnyBootstrapJs:          GetAssetsUrl(consts.AssetsJasnyBootstrapJs),
		PathFullcalendarJs:            GetAssetsUrl(consts.AssetsFullcalendarJs),
		PathJqueryJvectorMapJs:        GetAssetsUrl(consts.AssetsJqueryJvectorMapJs),
		PathNouisliderJs:              GetAssetsUrl(consts.AssetsNouisliderJs),
		PathCloudFareJs:               "https://cdnjs.cloudflare.com/ajax/libs/core-js/2.4.1/core.js",
		PathArriveJs:                  GetAssetsUrl(consts.AssetsArriveJs),
		PathChartistJs:                GetAssetsUrl(consts.AssetsChartistJs),
		PathChartMinJs:                GetAssetsUrl(consts.AssetsChartMinJs),
		PathChartMinCss:               GetAssetsUrl(consts.AssetsChartMinCss),
		PathUtilsJs:                   GetAssetsUrl(consts.AssetsUtilsJs),
		PathBootstrapNotifyJs:         GetAssetsUrl(consts.AssetsBootstrapNotifyJs),
		PathDemoJs:                    GetAssetsUrl(consts.AssetsDemoJs),
		PathImageSidebarJpeg1:         GetAssetsUrl(consts.AssetsImageSidebarJpeg1),
		PathImageSidebarJpeg2:         GetAssetsUrl(consts.AssetsImageSidebarJpeg2),
		PathImageSidebarJpeg3:         GetAssetsUrl(consts.AssetsImageSidebarJpeg3),
		PathImageSidebarJpeg4:         GetAssetsUrl(consts.AssetsImageSidebarJpeg4),
		PathSuiteJs:                   GetAssetsUrl(consts.AssetsSuiteJs) + "?v=6.5.5",
		PathSuiteCss:                  GetAssetsUrl(consts.AssetsSuiteCss) + "?v=6.5.5",
		PathGridCss:                   GetAssetsUrl(consts.AssetsGridCss) + "?v=6.5.5",
		PathDatasetJs:                 GetAssetsUrl(consts.AssetsDatasetJs) + "?v=6.5.5",
		CpModule:                      cpmod,
		//PathMaterialDashboardJs:       GetAssetsUrl(consts.AssetsMaterialDashboardJs) + "?v=2.1.2",
	}
}
...
// MakeConnectionString - делает строку соединения в зависимости от драйвера базы
func MakeConnectionString(driver string, config *ConfigSql) string {
	switch driver {
	case "mysql":
		return config.User + ":" + config.Password + "@tcp(" + config.Host + ":" + config.Port + ")/" + config.Name
	case "sqlite3":
