...
func XmlGenerator(www_dir string, mp *sqlpool.SqlPool) *worker.Worker {
...
func xml_loop(ctx context.Context, www_dir string, mp *sqlpool.SqlPool) {
...
func xml_detect(ctx context.Context, dir, host string, mp *sqlpool.SqlPool) {
	db := mp.Get(host)
	if db != nil {
