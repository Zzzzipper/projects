...
func (this *Modules) RegisterModule_Blog() *Module {
	return this.newModule(MInfo{
		...
		Order:  1,
		System: false,
		Icon:   "<i class=\"material-icons notranslate\">list</i>", //assets.SysSvgIconList,
		Sub: &[]MISub{
			{Mount: "default", Name: "Список публикаций", Show: true, Icon: "<i class=\"material-icons notranslate\">list</i>" /*assets.SysSvgIconList*/},
			{Mount: "add", Name: "Добавить публикацию", Show: true, Icon: "<i class=\"material-icons notranslate\">add</i>" /*assets.SysSvgIconPlus*/},
			...
			{Mount: "categories-add", Name: "Добавить новую", Show: true, Icon: "<i class=\"material-icons v\">add</i>" /*assets.SysSvgIconPlus*/},
		...
		if len(wrap.UrlArgs) == 3 && wrap.UrlArgs[0] == "blog" && wrap.UrlArgs[1] == "category" && wrap.UrlArgs[2] != "" {
			...
			err := wrap.DB.QueryRow(
				...
				`SELECT
					...
					blog_posts.briefly,
					blog_posts.content,
					strftime('%s', blog_posts.datetime) as datetime,
					blog_posts.active,
					users.id,
					users.first_name,
					users.last_name,
		...
		if wrap.CurrSubModule == "" || wrap.CurrSubModule == "default" {
			...
			content += builder.DataTable(
				...
				func(values *[]string) string {
					return builder.DataTableAction(&[]builder.DataTableActionRow{
						{
							Icon: `<i class="material-icons notranslate mr-1" style="color: #aae; font-size: 1.1em;">visibility</i>`, //assets.SysSvgIconView,
							//Icon:  `<button type="button" rel="tooltip" class="btn btn-info btn-link" data-original-title="" title="">
						...
						{
							Icon: `<i class="material-icons notranslate mr-1" style="color: #0b0; font-size: 1.1em;">create</i>`, //assets.SysSvgIconEdit,
							//Icon:  `<button type="button" rel="tooltip" class="btn btn-success btn-link" data-original-title="" title="">
						...
						{
							Icon: `<i class="material-icons notranslate" style="color: #e00; font-size: 1.1em;">delete</i>`, //assets.SysSvgIconRemove,
							//Icon:  `<button type="button" rel="tooltip" class="btn btn-danger btn-link" data-original-title="" title="">
			...
			content += this.getBreadCrumbs(wrap, &[]consts.BreadCrumb{
				{Name: "Категории", Link: "/cp/" + wrap.CurrModule + "/" + wrap.CurrSubModule + "/"},
				...
				},
				{
					...
					CallBack: func(field *builder.DataFormField) string {
						return `<div class="form-group n4">` +
							`<div class="row">` +
							`<label for="lbl_category" class="col-sm-3 col-form-label">Категория</label>` +
							`<div class="col-sm-9">` +
							`<select class="selectpicker" data-style="select-with-transition" id="lbl_category" name="category" data-live-search="true" data-width="45%">` +
				...
				},
				{
					...
					CallBack: func(field *builder.DataFormField) string {
						return `<div class="form-group n5">` +
							`<div class="row">` +
							`<label for="lbl_parent" class="col-sm-3 col-form-label">Все категории</label>` +
							`<div class="col-sm-9">` +
							`<select class="selectpicker" data-style="select-with-transition" id="lbl_cats" name="cats[]" data-live-search="true" multiple data-width="100%">` +
