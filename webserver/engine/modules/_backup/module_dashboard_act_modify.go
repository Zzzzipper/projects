...
func (this *Modules) RegisterAction_DashboardsModify() *Action {
	return this.newAction(AInfo{
		Mount:     "dashboard-modify",
		WantAdmin: true,
		...
		if pf_id == "0" {
			...
			wrap.Write(`window.location='/cp/dashboard/modify/` + utils.Int64ToStr(lastID) + `/';`)
		} else {
			// Update page
			if err := wrap.DB.Transaction(wrap.R.Context(), func(ctx context.Context, tx *wrapper.Tx) error {
				_, err := tx.Exec(
			...
			wrap.ResetCacheBlocks()
			wrap.Write(`window.location='/cp/dashboard/modify/` + pf_id + `/';`)
		}
	})
}
