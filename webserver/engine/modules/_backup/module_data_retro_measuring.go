...
func (this *Modules) RegisterData_RetroMeasuring() *Data {
	return this.newData(DInfo{
		Mount: "retro-measuring",
	},
		func(wrap *wrapper.Wrapper) {
			...
			if len(wrap.R.Header["Origin"]) > 0 {
				fmt.Println("Origin for measuring: ", wrap.R.Header["Origin"])
			...
			if conn != nil {
				retroData, err := conn.(gates.IGate).Measuring(m.Ids, m.StartTime, m.EndTime)
				if err != nil {
