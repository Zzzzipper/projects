...
func (this *Modules) RegisterModule_Templates() *Module {
	return this.newModule(MInfo{
		...
		Order:  802,
		System: true,
		Icon:   "<i class=\"material-icons notranslate\">visibility</i>", //assets.SysSvgIconView,
		Sub: &[]MISub{
			{Mount: "default", Name: "Редактор", Show: true, Icon: "<i class=\"material-icons notranslate\">list</i>"/*assets.SysSvgIconList*/},
			{Mount: "create", Name: "Добавить новый", Show: true, Icon: "<i class=\"material-icons notranslate\">add</i>"/*assets.SysSvgIconPlus*/},
			{Mount: "restore", Name: "Восстановить", Show: true, Icon: "<i class=\"material-icons notranslate\">restore_page</i>"/*assets.SysSvgIconRestore*/},
		},
					...
					},
					{
						...
						CallBack: func(field *builder.DataFormField) string {
							...
							if wrap.IsSystemMountedTemplateFile(selected_file) {
								buttons += `<button type="button" class="btn btn-success" onclick="return fave.ActionThemeFile('templates-restore-file','` + selected_file + `','Are you sure want to restore theme file?');">Восстановить</button>`
							} else {
								buttons += `<button type="button" class="btn btn-danger" onclick="return fave.ActionThemeFile('templates-delete-file','` + selected_file + `','Are you sure want to delete theme file?');">Удалить</button>`
							}

							return `<div class="form-group n1">` +
										`<div class="row">` +
											`<div class="col-sm-10">` +
												`<select class="selectpicker" data-style="select-with-transition" id="lbl_file" name="file" onchange="setTimeout(function(){$('#lbl_file').val('` + selected_file + `')},500);document.location='/cp/` + wrap.CurrModule + `/?file='+encodeURI(this.value);" data-width="100%">` +
													list_of_files +
												`</select>` +
											`</div>` +
