...
inline bool encrypt_plaintext(const crypto::peerkey_s &peerkey, const std::string &str_plaintext, oke::Ciphertext &ciphertext)
{
    ...
    ciphertext.cipher_version = CRYPTO_VERSION;
    ciphertext.aes_iv_12bytes = std::string((char*)rand_iv, CRYPTO_AES_IV_LEN);
    ciphertext.aes_tag_16bytes = std::string((char*)aes_tag, CRYPTO_AES_TAG_LEN);
    ciphertext.ciphertext_nbytes= std::move(str_ciphertext);
...
inline bool generate_token(const uint8_t ecdh_pub_key[CRYPTO_EC_PUB_KEY_LEN], oke::Token &token)
{
    ...
    token.salt_3bytes = std::string((char*)random_digit, 3);
    token.hmac_3bytes = std::string((char*)hmac_256, 3);
