...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()
            ...
            ("addr,a", value(&host), "tester server host ipv4 address")
            ("port,p", value(&port), "tester server port")
    ...
    try {
        ...
        // json::serializer< tester::request >::serialize(put, r);
        std::string buffer = put.str();

        LOG_TRACE << "Request: " << buffer;
